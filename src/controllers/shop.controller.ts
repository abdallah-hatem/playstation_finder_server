...
import {
  CreateShopDto,
  CreateShopWithImagesDto,
  UpdateShopWithImagesDto,
...
export class ShopController {
  ...
  @ApiBody({
    ...
    schema: {
      ...
      properties: {
        ...
        openingTime: {
          type: "string",
          description: "Shop opening time in HH:MM format",
          example: "09:00",
        },
        closingTime: {
          type: "string",
          description: "Shop closing time in HH:MM format",
          example: "22:00",
        },
        image: {
          type: "string",
  ...
  // @UseGuards(JwtAuthGuard, OwnerOnlyGuard)
  // @ApiBearerAuth('JWT-auth')
  // @ApiOperation({ summary: 'Get owner shops with pagination' })
  // @ApiPagination()
  // @ApiResponseSuccess({ message: 'Owner shops retrieved successfully' })
  // findMyShopsPaginated(@Query() paginationDto: PaginationDto, @CurrentOwner() owner: Owner) {
  //   return this.shopService.findByOwnerPaginated(owner.id, paginationDto);
  // }
  ...
  // @ApiQuery({
  //   name: "device",
  //   description: "Filter by device name (case-insensitive)",
  //   required: false,
  // })
  ...
  // @Get('all/paginated')
  // @Public()
  // @ApiOperation({ summary: 'Get all shops with pagination (public)' })
  // @ApiPagination()
  ...
  //   @Query('device') device?: string,
  // ) {
  //   return this.shopService.findAllPaginated(paginationDto, deviceId, device);
  // }
    ...
    schema: {
      ...
      properties: {
        ...
        images: {
          ...
          items: { type: "string", format: "binary" },
          description:
            "Replace shop gallery images (completely replaces existing)",
        },
      },
    },
  })
  ...
  updateWithImages(
    ...
    return this.shopService.updateWithImages(
      id,
      updateShopDto,
      files,
      owner.id
